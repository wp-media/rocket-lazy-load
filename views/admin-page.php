...
<div class="wrap rocket-lazyload-settings">
	...
	<div class="rocket-lazyload-header">
		...
		<p class="rocket-lazyload-rate-us">
			...
			printf( esc_html__( '%1$sDo you like this plugin?%2$s Please take a few seconds to %3$srate it on WordPress.org%4$s!', 'rocket-lazy-load' ), '<strong>', '</strong><br>', '<a href="' . esc_url( $rocket_lazyload_rate_url ) . '">', '</a>' );
			?>
			<br>
			<a class="stars" href="<?php echo esc_url( $rocket_lazyload_rate_url ); ?>"><?php echo str_repeat( '<span class="dashicons dashicons-star-filled"></span>', 5 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></a>
		</p>
	</div>
			...
			<fieldset>
				...
				<ul class="rocket-lazyload-options">
					...
					<li class="rocket-lazyload-option">
						...
						<label for="lazyload-<?php echo esc_attr( $rll_slug ); ?>">
							<span id="describe-lazyload-<?php echo esc_attr( $rll_slug ); ?>" class="rocket-lazyload-label-description"><?php echo esc_html( $rll_infos['label'] ); ?></span>
						</label>
					</li>
		...
		<div class="rocket-lazyload-upgrade">
			...
			<div class="rocket-lazyload-upgrade-cta">
				...
				<div class="rocket-lazyload-cta-block">
					<a class="button button-primary" href="https://wp-rocket.me/?utm_source=wp_plugin&utm_medium=rocket_lazyload"><?php esc_html_e( 'Get WP Rocket Now!', 'rocket-lazy-load' ); ?></a>
				</div>
			...
			<div class="rocket-lazyload-upgrade-arguments">
				<ul>
					...
					printf( esc_html__( '%1$sMultiple new features%2$s to further improve your load time', 'rocket-lazy-load' ), '<strong>', '</strong>' );
					?>
					</li>
					<li class="rll-upgrade-item">
					<?php
					// Translators: %1$s = strong opening tag, %2$s = strong closing tag.
					printf( esc_html__( 'All you need to %1$simprove your Google PageSpeed%2$s score', 'rocket-lazy-load' ), '<strong>', '</strong>' );
					?>
					</li>
					<li class="rll-upgrade-item">
					<?php
					// Translators: %1$s = strong opening tag, %2$s = strong closing tag.
					printf( esc_html__( '%1$sBoost your SEO%2$s by preloading your cache page for Google’s bots', 'rocket-lazy-load' ), '<strong>', '</strong>' );
					?>
					</li>
					<li class="rll-upgrade-item">
					<?php
					// Translators: %1$s = strong opening tag, %2$s = strong closing tag.
					printf( esc_html__( 'Watch your conversion rise with the %1$s100%% WooCommerce compatibility%2$s', 'rocket-lazy-load' ), '<strong>', '</strong>' );
					?>
					</li>
					<li class="rll-upgrade-item">
					<?php
					// Translators: %1$s = strong opening tag, %2$s = strong closing tag.
					printf( esc_html__( 'Minimal configuration, %1$sImmediate results%2$s', 'rocket-lazy-load' ), '<strong>', '</strong>' );
					?>
					</li>
					<li class="rll-upgrade-item">
					<?php
					// Translators: %1$s = strong opening tag, %2$s = strong closing tag.
					printf( esc_html__( 'Set up takes %1$s5 minutes flat%2$s', 'rocket-lazy-load' ), '<strong>', '</strong>' );
					?>
					</li>
					<li class="rll-upgrade-item">
					<?php
					// Translators: %1$s = strong opening tag, %2$s = strong closing tag.
					printf( esc_html__( '%1$s24/7 support%2$s', 'rocket-lazy-load' ), '<strong>', '</strong>' );
					?>
					</li>
				</ul>
